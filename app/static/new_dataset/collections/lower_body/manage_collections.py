...
import json, os, mimetypes, codecs
...
def addAllImagesFromFolder():
    if (this_collection_id==fashion_blogger_id):
        ...
        json_data = codecs.open("lower_body.json", "r", encoding='utf-8', errors='ignore')
        names_obj=utils.get_hashmap(json_data)
    ...
    for file in file_list:
            ...
            with open(url, 'rb') as img:   
                ...
                if(this_collection_id==fashion_blogger_id):                     
                    ...
                    try:
                        metadata=utils.get_metadata(names_obj, img.name)
                        visual_recognition.add_image(this_collection_id, img,metadata)
                        print(img)  
                    except:
...
#find similarities
with open(("tt00001.jpg"), 'rb') as img: 
